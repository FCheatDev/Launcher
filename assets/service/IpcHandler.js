...
class IpcHandler {
  ...
  _setupNavigationHandlers() {
    ipcMain.on("return-home", () => {
      ...
      if (mainWindow) {
        mainWindow
          .loadFile(path.join("assets/html/games.html"))
    ...
    ipcMain.on("open-external-link", (event, url) => {
      ...
      shell
        .openExternal(url)
        .catch((err) => logger.error("Failed to open external link:", err));
    });
  }
  /*
   * 主題
  ...
  _setupGameHandlers() {
    ...
    ipcMain.handle("download-executor", async (event, gameId) => {
      try {
        ...
        logger.system(
          `Starting download for ${config.name} from ${downloadUrl}`
        );
        ...
        if (isZip) {
          try {
            ...
            finalPath = path.join(
              config.installDir,
              path.basename(config.exePath)
            );
            await fs.move(path.join(tempExtractPath, exeFile), finalPath, {
              overwrite: true
            });
            ...
            await fs
              .remove(path.join(config.installDir, "temp_extract"))
              .catch(() => {});
            throw error;
          }
        } else {
        ...
        logger.system(
          `Installation completed for ${config.name} at ${finalPath}`
        );
        return finalPath;
      } catch (error) {
