...
class GameManager {
  ...
  checkGameInstalled(gameId) {
    try {
      ...
      logger.system(
        `${config.name} ${exists ? "found" : "not found"} at ${config.exePath}`
      );
      return {
        exists,
  ...
  async getSolaraUrl() {
    try {
      ...
      try {
        ...
        logger.system(
          "Current Solara URL is invalid, trying to get new one..."
        );
      }

      // 如果當前 URL 無效，嘗試獲取新的
      const mainResponse = await axios.get(
        "https://solaraweb.vercel.app/download",
        {
          maxRedirects: 5,
          headers: {
            "User-Agent":
      ...
      const match = text.match(
        /https:\/\/([\w\d]+)\.solaraweb-alj\.pages\.dev/
      );
      if (match) {
  ...
  async renameExecutorFile(gameId, tempPath) {
    ...
    const newFileName =
      renameMap[gameId.toUpperCase()] || path.basename(config.exePath);
  ...
  async launchGame(gameId) {
    try {
      ...
      if (gameId.toUpperCase() === "LUNA") {
          ...
          execFile(
            ...
            (error) => {
              if (error) {
                ...
                logger.system(
                  `Luna launched successfully from ${config.installDir}`
                );
                resolve(true);
              }
            }
        ...
        process.unref();
        logger.system(
          `${config.name} launched successfully from ${config.installDir}`
        );
        return true;
      }
    } catch (error) {
